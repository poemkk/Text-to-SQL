...
def normalize_sql_for_duckdb(sql):
    if not sql:
        return sql
...
def execute_duckdb(con, sql, apply_normalization=False):
    start = time.time()
    ...
    duckdb_sql = normalize_sql_for_duckdb(sql) if apply_normalization else sql
...
Your task is to produce multiple DuckDB-compatible SQL candidates while preserving the same meaning.
...
Repair rules:
- Return {n_candidates} diverse DuckDB-compatible SQL candidates when possible.
...
def main():
    ...
    with out_path.open(write_mode, encoding="utf-8") as out:
        for row_idx, item in enumerate(rows, start=1):
            ...
            item["dialect_repair_stage"] = "none"
            item["llm_dialect_repair_attempted"] = False

            raw_exec_ok = bool(item.get("duckdb_exec_ok"))
            raw_same = bool(item.get("crossdb_same_result"))
            ...
            norm_exec_ok = item.get("duckdb_normalized_exec_ok")
            norm_result = item.get("duckdb_normalized_result")
            norm_error = item.get("duckdb_normalized_exec_error")
            norm_sql = item.get("duckdb_normalized_sql")

            if norm_exec_ok is None:
                norm_validation = execute_duckdb(
                    con,
                    sqlite_sql,
                ...
                norm_sql = norm_validation["duckdb_sql"]
                item["duckdb_normalized_attempted"] = True
                item["duckdb_normalized_exec_ok"] = norm_exec_ok
                item["duckdb_normalized_exec_error"] = norm_error
                item["duckdb_normalized_result"] = norm_result
                item["duckdb_normalized_latency_ms"] = norm_validation["latency_ms"]
                item["duckdb_normalized_sql"] = norm_sql
            ...
            if norm_same:
                ...
                item["dialect_repair_stage"] = "normalize"
                item["dialect_repair_sql"] = norm_sql
                item["dialect_repair_exec_ok"] = True
                item["dialect_repair_exec_error"] = None
                item["dialect_repair_result"] = norm_result
            ...
            if raw_exec_ok and not raw_same:
                mismatch_hint = build_result_mismatch_hint(sqlite_result, item.get("duckdb_result"))
                duckdb_error = "result_mismatch_after_execution"
            else:
                mismatch_hint = None
                duckdb_error = norm_error or item.get("duckdb_exec_error")
            ...
            for round_idx in range(1, args.max_repair_rounds + 1):
                ...
                llm_api_calls += 1
                candidates, raw_response = call_dialect_repair(
                    client=client,
                    model=args.model,
                    db_id=db_id,
                ...
                if best_validation and best_validation["exec_ok"]:
                    ...
                    duckdb_error = best_validation["exec_error"]
            ...
            item["dialect_repair_attempted"] = True
            item["dialect_repair_stage"] = "llm"
            item["llm_dialect_repair_attempted"] = True
            item["dialect_repair_rounds_used"] = rounds_used
            item["dialect_repair_candidates"] = candidate_records
            item["dialect_repair_candidate_count"] = len(candidate_records)
            item["dialect_repair_first_candidate_sql"] = first_candidate_record["sql"]
            item["dialect_repair_first_candidate_exec_ok"] = first_candidate_record["exec_ok"]
            item["dialect_repair_first_candidate_error"] = first_candidate_record["exec_error"]
            item["dialect_repair_first_candidate_same_result"] = first_candidate_record["same_result"]
            item["dialect_repair_select_attempted"] = True
            item["dialect_repair_select_policy"] = "same_result_then_exec_then_row_count_delta"
            item["dialect_repair_selected_sql"] = best_sql
            item["dialect_repair_selected_score"] = list(best_score)
            item["dialect_repair_selected_exec_ok"] = best_validation["exec_ok"]
            item["dialect_repair_selected_error"] = best_validation["exec_error"]
            item["dialect_repair_selected_same_result"] = best_same_result
            item["dialect_repair_sql"] = best_sql
            item["dialect_repair_raw_response"] = "\n\n".join(all_raw_responses)
            item["dialect_repair_exec_ok"] = best_validation["exec_ok"]
            item["dialect_repair_exec_error"] = best_validation["exec_error"]
            item["dialect_repair_result"] = best_validation["result"]
            item["dialect_repair_latency_ms"] = best_validation["latency_ms"]
            item["dialect_repair_same_result"] = best_same_result
            ...
            if (
                ...
                print(
                    f"[PROGRESS] duckdb triggered={repair_triggered} "
    ...
    print(f"before DuckDB executable (raw SQL): {before_ok}/{total} = {before_ok / total:.3f}")
