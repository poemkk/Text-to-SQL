...
class PairwiseRankerTrainer(Trainer):
    @staticmethod
    def _split_pairwise_inputs(inputs, device=None):
        """Convert pos_*/neg_* batch fields into normal model input fields."""
        pos_inputs = {}
        neg_inputs = {}
        ...
        for key, value in inputs.items():
            ...
            if key.startswith("pos_"):
                model_key = key.removeprefix("pos_")
                pos_inputs[model_key] = value.to(device) if device is not None else value
            elif key.startswith("neg_"):
                model_key = key.removeprefix("neg_")
                neg_inputs[model_key] = value.to(device) if device is not None else value
    ...
    def prediction_step(self, model, inputs, prediction_loss_only, ignore_keys=None):
        """
        Custom evaluation step for pairwise ranking batches.

        HuggingFace Trainer's default prediction_step calls model(**inputs).
        That fails here because the batch contains fields such as pos_input_ids
        and neg_input_ids, while CodeBERT/Roberta expects input_ids.
        """
        model.eval()
        pos_inputs, neg_inputs = self._split_pairwise_inputs(inputs, device=model.device)
        ...
        # Shape: [batch_size, 2]. Column 0 should be larger than column 1.
...
def compute_pairwise_metrics(eval_pred):
    ...
    return {"pairwise_accuracy": float((preds == labels).mean())}
...
def main():
    ...
    trainer = PairwiseRankerTrainer(
        ...
        tokenizer=tokenizer,
        compute_metrics=compute_pairwise_metrics if dev_dataset else None,
    )
