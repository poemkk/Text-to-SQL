...
DOCX_PATH = OUTPUT_DIR / "第8章_原型系统实现_讨论与结论.docx"
MD_PATH = ROOT / "docs" / "chapter8_prototype_system.md"
FIGURE_DIR = ROOT / "output" / "playwright"
...
FIGURES = [
    (
        "图 8.1 A²V-SQL 原型系统主界面",
        FIGURE_DIR / "fig8_1_main_interface.png",
        Inches(6.0),
        "8.2 原型系统总体架构",
    ),
    (
        "图 8.2 A²V-SQL 原型系统总体架构与核心流程",
        FIGURE_DIR / "fig8_2_schema_and_candidates.png",
        Inches(6.0),
        "8.2 原型系统总体架构",
    ),
    (
        "图 8.3 原型系统主界面与输入入口",
        FIGURE_DIR / "fig8_1_control_panel.png",
        Inches(6.0),
        "8.5 原型系统展示案例",
    ),
    (
        "图 8.4 Schema context、候选 SQL 与验证轨迹展示界面",
        FIGURE_DIR / "fig8_2_candidate_trace_focus.png",
        Inches(5.8),
        "8.5 原型系统展示案例",
    ),
    (
        "图 8.5 SQL 执行验证与错误反馈修复界面",
        FIGURE_DIR / "fig8_3_validation_repair.png",
        Inches(5.8),
        "8.5 原型系统展示案例",
    ),
    (
        "图 8.6 EASE 最终选择与执行结果展示界面",
        FIGURE_DIR / "fig8_4_selector_and_result.png",
        Inches(6.0),
        "8.5 原型系统展示案例",
    ),
]
...
def add_figure(document: Document, caption: str, path: Path, width):
    if not path.exists():
        return
    p = document.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
...
def build_docx():
    ...
    for heading, paragraphs in SECTIONS:
        ...
        for caption, path, width, target_heading in FIGURES:
            if target_heading == heading:
                add_figure(doc, caption, path, width)

    doc.save(DOCX_PATH)


def build_markdown():
    lines = ["# 8. 原型系统实现、讨论与结论", ""]
    ...
    for heading, paragraphs in SECTIONS:
        ...
        for caption, path, _, target_heading in FIGURES:
            if target_heading == heading:
